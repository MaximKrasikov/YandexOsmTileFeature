package com.avpower.yandexosmtilefeature.util;

/** Утилита для преобразования слоёв карты из Yandex в OSM(Google) */
public class YandexUtil  {

    public static double[] tileNumberToCoordinates(int tileX, int tileY, int mapZoom){
        double n = Math.pow(2.0, (double) mapZoom);
        double lon = ((double) tileX /n) * 360.0 - 180.0;
        double lat = Math.atan( Math.sinh(Math.PI - ((double)tileY/n) * 2 * (double)Math.PI)) *(180.0/Math.PI);

        return new double[]{lat,lon};
    }
    public static int[] getWgs84MercatorValue(double latitude, double longitude, int zoom) {
        int x, y;
        double
                rho,
                pi = Math.PI,
        ...
        x = (int)(rho * (1 + longitude / 180));
        y = (int)(rho * (1 - Math.log(theta) / pi));

        return new int[]{x, y};
    }

    public static double[] geoToMercator(double[] g) { //WGS84ToBing
    ...
    public static double[] mercatorToGeo(double[] e) {//BingtoWGS84Mercator
    ...
    public static int[] getMapTileFromCoordinates(final double aLat, final double aLon, final int zoom) {
        ...
        out[0] = (int) Math.floor(B2 / 2 - M2 * B2 / 2 / Math.PI); //y

        out[1] = (int) Math.floor((aLon + 180) / 360 * (1 << zoom)); //x


        out[2] = (int) Math.floor(((aLon+180)/360 * B2 - out[1])*256); //offsetX
        out[3] = (int) Math.floor(((B2/2-M2 * B2/2/(double)Math.PI) - out[0])*256); //offsetY
